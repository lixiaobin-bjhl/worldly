...
//定义一个命令
exports.command = function(cname, discribe, processer, auto) {
...
function autoCompleter(p) { //p是用户输入的字符串(包括前后的空格)
	...
	if (args[0] in commands) {
		...
		var cnames = [];
		for (var item in commands_list) {
			cnames.push(commands_list[item].cname);
		}
		return [filter(p, cnames, '', ' '), p];
	}
}
